...
import { enqueueProjectPullSync } from '@/lib/minio-pull-queue';
...
const log = createLogger('MinioPullSyncAPI');
...
// POST /api/sync/minio/pull - Enqueue MinIO pull sync job by projectId
export async function POST(request: NextRequest) {
  try {
    ...
    const enqueueResult = await enqueueProjectPullSync(project.path, project.id);

    return NextResponse.json(
      {
        success: true,
        accepted: true,
        message: 'MinIO pull sync queued',
    ...
    log.error({ err: error }, 'Failed to trigger MinIO pull sync');
    return NextResponse.json({ error: 'Failed to trigger MinIO pull sync' }, { status: 500 });
  }
}
